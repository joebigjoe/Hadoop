...
print(rdd_movies_with_key.collect())
...
rdd_ratings_with_movieidaskey = rdd_ratings.map(lambda token: (token.split(',')[1], token))
print(rdd_ratings_with_movieidaskey.collect())

# join the data
rdd_joined = rdd_movies_with_key.join(rdd_ratings_with_movieidaskey)
print(rdd_joined.collect())

# left join and right join is similiar to the DB operation.
# skip for now
