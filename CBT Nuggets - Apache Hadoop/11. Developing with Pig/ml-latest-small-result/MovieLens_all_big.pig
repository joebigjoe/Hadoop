...
movies = LOAD 'hdfs:/user/hadoopuser/moviedata/ml-latest/movies_nocomma.csv' USING PigStorage(',') AS (movieId:int, title:chararray, genres:chararray);
ratings = LOAD 'hdfs:/user/hadoopuser/moviedata/ml-latest/ratings.csv' USING PigStorage(',') AS (userId:int, movieId:int, rating:int, timestamp:chararray);
...
--STORE genresforuser_bagtostring into 'hdfs:/user/hadoopuser/moviedata/ml-latest-small/genresforuser_bagtostring1'; 
...
--dump ordered;
STORE ordered into 'hdfs:/user/hadoopuser/moviedata/ml-latest/best_for_now_big';